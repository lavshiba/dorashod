...
В текущих последних проходах под frozen-подачу дополнительно подтянуты заметные пользовательские экраны `поиск`, `отчёт`, `категории`, `настройки` и `данные`, включая более человечный формат дат в списках и более точные подтверждения в сценариях загрузки / сброса / очистки.
