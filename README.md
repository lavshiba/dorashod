...
В текущем проходе под frozen-подачу дополнительно подтянуты заметные пользовательские экраны `поиск`, `отчёт` и `категории`, включая более человечный формат дат в списках.
